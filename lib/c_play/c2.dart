...
class _C2State extends State<C2> {
  ...
  List<bool> _visibilityAfter = List.generate(0, (index) => false);
  List<List<double>> _childLocation =
  List.generate(0, (i) => List.generate(2, (j) => 0));
  List<bool> _position = List.generate(5, (index) => false);
  List<int> _nowPosition = List.generate(0, (index) => 0);
  int childCount = 0;
  ...
  void setPosition(int positionNumber, bool isThere){
    _position[positionNumber] = isThere;
  ...
  void _changeHide(int listNumber, bool objectStatus) {
    ...
    setState(() {
      ...
      _childLocation[listNumber] = childImageLocation[checkPosition()];
      _nowPosition[listNumber] = checkPosition();
      _visibilityAfter[listNumber] = true;
      setPosition(checkPosition(), true);
  ...
  void _changeShow(int listNumber, bool objectStatus, int position) {
    setState(() {
  ...
  void initState() {
    ...
    for (int i = 0; i < childs; i++) {
      _childLocation.add(childImageLocation[0]);
      _nowPosition.add(0);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; i < childs; i++) {
      ...
      _afterClickChild.add(AfterClickChild(
        ...
        childImageLocationX: _childLocation[i][1],
        childImageLocationY: _childLocation[i][0],
        childImagePosition: _nowPosition[i],
    ...
    return Scaffold(
        ...
        body: Stack(
          children: [
                  ...
                  children: [
                    Positioned(
                      child: Container(
                        ...
                        child: Image.asset(
                            'assets/backgrounds/BirthdayScene_01-4_BG.png'),
                      ),
                    ),
                  ],
                )),
            ...
            // Positioned(
            //   top:
            //       670 * _letterBoxSize.heightRatio!
            //           + _letterBoxSize.heightAdd!,
            //   left:
            //       1080 * _letterBoxSize.widthRatio!
            // + _letterBoxSize.widthAdd!,
            //   child: Container(
            //     child: Stack(
            //       children: [
            //         Container(
            //           width: 168 * MediaQuery.of(context).size.width / 2560,
            //           child:
            ...
            //   ),
            // ),
            // for (int i = 0; i < childs; i++) _afterClickChild[i],
          ],
        ));
  }
}
...
class AfterClickChild extends StatefulWidget {
  const AfterClickChild({
    ...
    required this.childImageLocationX,
    required this.childImageLocationY,
    required this.childImagePosition,
  }) : super(key: key);
  final Image childFace;
  final Function(int, bool, int)? notifyParent;
  final bool visibility;
  final int childImageNumber;
  final int clickCount;
  final double childImageLocationX;
  final double childImageLocationY;
  final int childImagePosition;
...
class _AfterClickChildState extends State<AfterClickChild> {
  ...
  Widget build(BuildContext context) {
    ...
    return Positioned(
      top: widget.childImageLocationY * _letterBoxSize.heightRatio! +
          _letterBoxSize.heightAdd!,
      left: widget.childImageLocationX * _letterBoxSize.widthRatio! +
          _letterBoxSize.widthAdd!,
      child: InkWell(
        child: Visibility(
          child: Stack(
            children: [
              ...
              Container(
                width: 144 * MediaQuery.of(context).size.width / 2560,
                margin: EdgeInsets.only(left: childImageInfo[0], top: 0),
                child: WidgetMask(
                  blendMode: BlendMode.srcATop,
                  childSaveLayer: true,
                  mask: widget.childFace,
        ...
        onTap: () {
          setState(() {
            widget.notifyParent!(widget.childImageNumber, false, widget.childImagePosition);
          });
        },
      ),
    );
